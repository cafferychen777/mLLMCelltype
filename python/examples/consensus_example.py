...
def run_consensus_example():
    ...
    if 'anthropic' in api_keys:
        models.append('claude-3-5-sonnet-20241022')  # 最新的Claude模型
    if 'gemini' in api_keys:
        models.append('gemini-1.5-pro')  # 不带models/前缀的Gemini模型名称
    if 'qwen' in api_keys:
        models.append('qwen2.5-72b-instruct')  # Alibaba的Qwen模型
    ...
    result = lct.interactive_consensus_annotation(
        ...
        max_discussion_rounds=3,  # 启用最多3轮迭代讨论
        use_cache=True,
        verbose=True
    )
