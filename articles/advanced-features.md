...
Cell types often exist in hierarchical relationships. For example, T
cells can be further classified into CD4+ T cells, CD8+ T cells,
regulatory T cells, etc. mLLMCelltype can be used in a multi-step
workflow to capture these hierarchical relationships.
...
After creating hierarchical annotations, it’s important to validate the
...
Real-world scRNA-seq data often contains noise. Here are practical
strategies for handling noisy input:

#### 1. Adjust the top_gene_count parameter

For noisy datasets, using fewer top genes can help focus on the
strongest signals:
...
Pre-filtering marker genes with stricter thresholds can improve
annotation quality:
...
The consensus approach can help overcome noise by combining predictions
...
For advanced use cases, you can create and modify the annotation prompt
directly:
...
You can enhance your annotation workflow by combining mLLMCelltype with
...
This example demonstrates a complete workflow for analyzing a PBMC
dataset:
...
When working with datasets containing rare cell populations, you can
...
When using mLLMCelltype, it’s important to consider the costs associated
...
For advanced users, mLLMCelltype allows you to register custom providers
and models:
...
mLLMCelltype provides a comprehensive unified logging system with
...
The `CacheManager` class helps optimize performance by caching consensus
results:
...
mLLMCelltype provides convenient functions for managing cache
directories:
