...
cell type annotation in single-cell RNA sequencing data.

### General Questions

#### What makes mLLMCelltype different from other cell type annotation tools?

mLLMCelltype differs from traditional cell type annotation tools in
several key ways:

1.  **No reference dataset required**: Unlike reference-based methods,
    mLLMCelltype doesn’t require a pre-existing reference dataset.

2.  **Multi-model consensus**: mLLMCelltype leverages multiple large
    language models to achieve more reliable annotations than any single
    model could provide.

3.  **Transparent reasoning**: The package provides complete reasoning
    chains for annotations, making the process interpretable and
    transparent.

4.  **Uncertainty quantification**: mLLMCelltype provides explicit
    uncertainty metrics (consensus proportion and Shannon entropy) to
    identify ambiguous cell populations.

5.  **Structured deliberation**: For controversial clusters,
    mLLMCelltype initiates a structured discussion process among models
    to reach a more reliable consensus.

#### Which tissues and species does mLLMCelltype support?

mLLMCelltype can annotate cell types from virtually any tissue and
...
The package has been extensively tested on: - Human tissues (PBMC, bone
marrow, brain, lung, liver, kidney, etc.) - Mouse tissues (brain, lung,
kidney, etc.) - Other model organisms (zebrafish, fruit fly, etc.)

For very specialized or poorly characterized tissues, the uncertainty
metrics will help identify clusters that may require expert review.

#### How accurate is mLLMCelltype compared to other methods?

In our benchmarking studies (see our
[paper](https://doi.org/10.1101/2025.04.10.647852)), mLLMCelltype
consistently outperformed both traditional annotation methods and
single-LLM approaches:

- Compared to reference-based methods (e.g., SingleR, Seurat label
  transfer), mLLMCelltype showed comparable or better performance
  without requiring a reference dataset.
- Compared to marker-based methods (e.g., SCINA, CellAssign),
  mLLMCelltype demonstrated higher accuracy and flexibility.
- Compared to single-LLM approaches, the consensus mechanism improved
  accuracy by 15-30% depending on the tissue type.

The accuracy advantage is particularly pronounced for rare cell types
and tissues with limited reference data.
...
- For well-characterized cell types: 5-10 marker genes is usually
  sufficient
- For rare or poorly characterized cell types: 10-20 marker genes may be
  beneficial
- For noisy data: Fewer genes (5-7) might give better results by
  focusing on the strongest signals

The optimal number depends on the quality of your marker genes and the
complexity of the tissue. We recommend starting with the default of 10
and adjusting based on the results.
...
- Detailed error messages: When an API call fails, the error message
  includes details to help diagnose the issue

If you’re processing many clusters, you might encounter rate limits. In
this case:
...
- **Discussion process**: Controversial clusters require additional API
  calls for discussion
- **API response times**: Different providers have different response
  times
- **Network conditions**: Internet speed and reliability affect
  performance

Typical runtimes: - Single model, 10 clusters: 1-2 minutes - Multi-model
...
For a typical dataset with 10-20 clusters: - Single model annotation:
\$0.10-1.00 total - Multi-model consensus (3 models): \$0.30-3.00
...
1.  **Sign up for an OpenRouter account** at
...
**Important Notes about Free Models** (Updated Oct 2025): - **Daily
limits reduced**: 50 requests/day for free accounts (down from 200) -
**Accounts with \$10+ credits**: 1000 requests/day - **Rate limit**: 20
requests/minute for all accounts - **Some models removed**: NVIDIA
Nemotron and others have exited the free tier - **Availability may
change**: Verify current free models at
<https://openrouter.ai/models?q=free> - **For production use**: Consider
...
3.  **Specify the correct tissue**: Always provide the correct tissue
    name to give models the proper context

4.  **Review uncertainty metrics**: Pay attention to consensus
    proportion and Shannon entropy to identify clusters that may need
    manual review

5.  **Examine discussion logs**: For controversial clusters, review the
    discussion logs to understand the reasoning

6.  **Iterate if needed**: If results are unsatisfactory, try adjusting
    parameters or providing additional context

### Troubleshooting

#### Why am I getting different results with the same input?

There are several possible reasons for getting different results with
the same input:

1.  **Model updates**: LLMs are regularly updated, which can change
    their outputs

2.  **Temperature/sampling**: Some randomness is inherent in LLM outputs

3.  **Context window limitations**: Different runs might include
...
1.  **Check environment variables**: Ensure your API keys are set
    correctly in your `.env` file or environment

2.  **Provide keys directly**: Pass the API key directly to the
    function:
...
3.  **Check provider name**: Make sure you’re using the correct provider
...
4.  **Verify key validity**: Check if your API key is still valid by
    testing it directly with the provider’s API

#### Why are some cell types not being correctly identified?

If specific cell types are not being correctly identified:

1.  **Check marker genes**: Ensure the marker genes for these cell types
    are strong and specific

2.  **Provide more context**: Specify the tissue type accurately to give
    models the right context

3.  **Use more models**: Different models have different strengths;
    using multiple models improves coverage

4.  **Increase marker count**: Try increasing `top_gene_count` to
    include more marker genes

5.  **Review discussion logs**: For controversial clusters, examine the
    discussion to understand the reasoning

6.  **Consider rare cell types**: Some cell types may be poorly
    represented in the training data of LLMs
...
3.  **Visualization**: Use Seurat’s visualization functions with the
    added annotations:
...
1.  **Extract marker genes**: Export marker genes from your Scanpy
    analysis to a CSV file

2.  **Run mLLMCelltype**: Use the CSV file as input to mLLMCelltype

3.  **Import results**: Add the annotation results back to your AnnData
    object

Alternatively, you can use the Python version of mLLMCelltype for direct
...
2.  **Validation**: Use mLLMCelltype to validate annotations from
    reference-based methods

3.  **Hybrid approach**: Use reference-based methods for
    well-characterized cell types and mLLMCelltype for novel or rare
    cell types

4.  **Ensemble method**: Create a consensus between mLLMCelltype and
    traditional methods
...
While mLLMCelltype uses carefully designed prompts, advanced users can
customize them:
...
1.  **Report issues**: Report bugs or suggest features on our [GitHub
    repository](https://github.com/cafferychen777/mLLMCelltype/issues)

2.  **Improve documentation**: Help us improve documentation and
    examples

3.  **Add new models**: Implement support for new LLM models

4.  **Share benchmarks**: Share your benchmarking results with different
    tissues and species

5.  **Develop new features**: Contribute code for new features or
    improvements
