...
While mLLMCelltype focuses on cell type annotation, you can create
marker gene visualizations using standard Seurat functions. These
visualizations help validate the consensus annotations by showing marker
gene expression patterns across annotated cell types.
...
The dotplot is a popular visualization that shows marker gene expression
across cell types with both expression level (color) and expression
percentage (dot size):
...
Create a classic heatmap showing marker gene expression across cell
types:
...
- **Rows**: Marker genes (hierarchically clustered by expression
  pattern)
...
For a quick analysis with predefined key markers, you can create a
simplified version by selecting specific genes of interest and using a
simpler color scheme. This approach is faster when you already know
which markers to focus on.

#### Marker Gene Visualization Best Practices

1.  **Gene Selection**: Select top 3-5 marker genes per cell type for
    clarity; too many genes can make plots cluttered
2.  **Color Palettes**:
    - Use `viridis` or sequential color palettes for dotplots
    - Use blue-white-red diverging palettes for heatmaps (intuitive for
      expression)
    - Consider colorblind-friendly palettes
3.  **Scaling**: Row-wise scaling is recommended for heatmaps when using
    `pheatmap` or `ComplexHeatmap`
4.  **Dotplots**: Use Seurat’s `DotPlot` function with appropriate dot
    size scaling and clear color gradients
5.  **File Formats**: Save as PNG for presentations, PDF for
    publications
