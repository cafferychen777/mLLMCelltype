...
cell type annotation in single-cell RNA sequencing data. We’ll cover
various usage scenarios, parameter configurations, and integration with
Seurat.
...
Function for creating consensus annotations from multiple models through
...
**Important Note on `consensus_check_model`**: This parameter is
critical for annotation accuracy. The consensus check model evaluates
semantic similarity, calculates consensus metrics, and moderates
discussions. We strongly recommend using high-performance models like: -
`claude-sonnet-4-5-20250929` (best overall, latest and most
intelligent) - `claude-opus-4-1-20250805` (excellent for complex
reasoning) - `o1` (advanced reasoning) - `gpt-5` (strong performance) -
...
mLLMCelltype supports a wide range of LLM models. Here’s a guide to help
you choose:
...
- **Anthropic Claude Sonnet 4.5** (`claude-sonnet-4-5-20250929`): Latest
  and most intelligent, excellent biological knowledge
- **Anthropic Claude Opus 4.1** (`claude-opus-4-1-20250805`): Best for
  complex reasoning and detailed analysis
- **OpenAI GPT-5** (`gpt-5`): Strong overall performance, good
  biological knowledge
- **Google Gemini 2.5 Pro** (`gemini-2.5-pro`): Top-tier performance
...
These free models are accessed through OpenRouter and don’t consume
credits, but may have limitations compared to paid models. Use the
...
Here’s a complete example of integrating mLLMCelltype with a Seurat
workflow:
...
The `top_gene_count` parameter controls how many top marker genes per
...
The `controversy_threshold` parameter in the
`interactive_consensus_annotation` function controls which clusters are
...
3.  Use a higher `controversy_threshold` to reduce the number of
    clusters that require discussion
...
- Ensure you’re using the correct format for the model name (e.g.,
  `provider/model-name:free`)
- Try a different free model, as some models may have access
  restrictions
...
- This typically occurs when using non-English language models like
  `thudm/glm-z1-9b:free` for consensus checking
- Switch to an English-optimized model like
  `meta-llama/llama-4-maverick:free` for consensus checking
...
Now that you understand the detailed usage of mLLMCelltype, you can
explore:
