...
# Set output directory for inference mode
...
print("\nInitializing PopV processor with inference mode...")
processor = popv.preprocessing.Process_Query(
    ...
    prediction_mode='inference',  # Inference mode for detailed prediction
...
# Update categories in cell_type column to include 'unknown'
cell_type_categories = list(ref_adata.obs['cell_type'].unique()) + ["unknown"]
processor.adata.obs['cell_type'] = processor.adata.obs['cell_type'].astype('category').cat.add_categories(["unknown"])
processor.adata.uns["label_categories"] = np.array(cell_type_categories)
...
methods = [
    ...
    "SCANVI_POPV"  # scANVI semi-supervised model
]
...
processor.adata.write_h5ad(os.path.join(output_dir, "Thymus_popv_inference.h5ad"))
...
predictions.to_csv(os.path.join(output_dir, "Thymus_popv_inference_results.csv"))
...
print("- Full annotated data: Thymus_popv_inference.h5ad")
print("- Prediction results: Thymus_popv_inference_results.csv")
