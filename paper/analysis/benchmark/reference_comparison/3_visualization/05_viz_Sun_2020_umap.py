...
# 设置路径
base_dir = "."
data_path = os.path.join(base_dir, "data/raw/Sun_2020.h5ad")
llm_results_path = os.path.join(base_dir, "results/benchmark/reference_comparison/2_evaluation/sun_2020_results.csv")
output_dir = os.path.join(base_dir, "results/figures/extended_figure9")
...
texts2 = []
added_labels = set()  # 用于跟踪已添加的标签
for ct in sorted(adata.obs['LLMCellType'].unique()):
    if pd.notna(ct) and ct != 'Unknown' and ct != 'nan':  # 排除nan值
        mask = adata.obs['LLMCellType'] == ct
        standardized_label = standardized_labels.get(ct, ct)

        # 检查标签是否已添加
        if standardized_label in added_labels:
            continue
        ...
        if np.sum(mask) > 0:
            ...
            texts2.append(ax2.text(centroid[0], centroid[1], standardized_label,
                     fontsize=8, ha='center', va='center'))
            added_labels.add(standardized_label)  # 记录已添加的标签
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'extended_figure9.pdf'),
            bbox_inches='tight', dpi=300)
plt.savefig(os.path.join(output_dir, 'extended_figure9.png'),
            bbox_inches='tight', dpi=300)
plt.close()
