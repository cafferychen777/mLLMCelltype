# Evaluation script for Pancreas cell type benchmark
library(LLMCellType)
...
unlink(file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/pancreas_results.rds"))
...
markers_list <- readRDS(file.path(project_root, "results/benchmark/reference_comparison/1_preprocessing/pancreas_markers.rds"))
...
results <- interactive_consensus_annotation(
  input = markers_list,
  tissuename = "human pancreas tissue",
...
# Save results
saveRDS(results, file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/pancreas_results.rds"))

print("Evaluation complete. Results saved to pancreas_results.rds")
