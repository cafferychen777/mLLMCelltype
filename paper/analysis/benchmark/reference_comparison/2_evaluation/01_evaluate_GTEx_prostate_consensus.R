# Evaluation script for GTEx Prostate cell type benchmark
library(LLMCellType)
...
unlink(file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/GTEx_prostate_results.rds"))
...
markers_list <- readRDS(file.path(project_root, "results/benchmark/reference_comparison/1_preprocessing/GTEx_prostate_markers.rds"))
...
for (i in 1:num_parts) {
  ...
  run_with_retry <- function(attempt = 1, max_attempts = 3) {
    tryCatch({
      results <- interactive_consensus_annotation(
        input = part_markers,
        tissuename = sprintf("human prostate
         tissue part %d", i),
  ...
  saveRDS(results,
          file.path(project_root,
                    sprintf("results/benchmark/reference_comparison/2_evaluation/GTEx_prostate_results_part%d.rds", i)))

  print(sprintf("Part %d evaluation complete. Results saved to GTEx_prostate_results_part%d.rds", i, i))
}
...
# Save merged results
combined_file <- file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/GTEx_prostate_results.rds")
