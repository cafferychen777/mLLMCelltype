# Evaluation script for Fetal Development cell type benchmark
library(LLMCellType)
...
unlink(file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/fetal_development_results.rds"))
...
markers_list <- readRDS(file.path(project_root, "results/benchmark/reference_comparison/1_preprocessing/fetal_development_markers.rds"))
...
for (i in 1:num_parts) {
  ...
  results <- interactive_consensus_annotation(
    input = part_markers,
    tissuename = sprintf("human fetal development tissue part %d", i),
  ...
  saveRDS(results,
          file.path(project_root,
                    sprintf("results/benchmark/reference_comparison/2_evaluation/fetal_development_results_part%d.rds", i)))

  print(sprintf("Part %d evaluation complete. Results saved to fetal_development_results_part%d.rds", i, i))
}
...
# Save merged results
combined_file <- file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/fetal_development_results.rds")
