# Evaluation script for PBMC cell type benchmark
library(LLMCellType)
...
unlink(file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/pbmc_results.rds"))
...
markers_list <- readRDS(file.path(project_root, "results/benchmark/reference_comparison/1_preprocessing/pbmc_markers.rds"))
...
results <- interactive_consensus_annotation(
  input = markers_list,
  tissuename = "human peripheral blood mononuclear cells (PBMC)",
...
# Save results
saveRDS(results, file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/pbmc_results.rds"))

print("Evaluation complete. Results saved to pbmc_results.rds")
