# Evaluation script for motor cortex cell type benchmark
library(LLMCellType)
...
unlink(file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/motor_cortex_results.rds"))
...
markers_list <- readRDS(file.path(project_root, "results/benchmark/reference_comparison/1_preprocessing/motor_cortex_markers.rds"))
...
tryCatch({
  results <- interactive_consensus_annotation(
    input = markers_list,
    tissuename = "human motor cortex tissue",
  ...
  print(paste("Error in consensus annotation:", e$message))
  print("Saving error details to error_log.txt")
  cat(sprintf("\nError occurred at %s\nError message: %s\nCall stack:\n%s",
              Sys.time(), e$message, paste(capture.output(traceback()), collapse="\n")),
      file=file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/logs/error_log.txt"),
      append=TRUE)
  stop(e)
})

# Save results
saveRDS(results, file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/motor_cortex_results.rds"))

print("Evaluation complete. Results saved to motor_cortex_results.rds")
