...
markers_list <- readRDS(file.path(project_root, "results/benchmark/reference_comparison/1_preprocessing/MCA_markers.rds"))
...
for (i in 1:num_parts) {
  ...
  results <- interactive_consensus_annotation(
    input = part_markers,
    tissuename = sprintf("Mouse mixed tissue part %d", i),
  ...
  saveRDS(results,
          file.path(project_root,
                    sprintf("results/benchmark/reference_comparison/2_evaluation/MCA_results_part%d.rds", i)))

  print(sprintf("Part %d evaluation complete. Results saved to MCA_results_part%d.rds", i, i))
}
...
# Save merged results
combined_file <- file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/MCA_results.rds")
