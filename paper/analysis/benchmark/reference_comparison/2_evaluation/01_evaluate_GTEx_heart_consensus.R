# Evaluation script for GTEx Heart cell type benchmark
library(LLMCellType)
...
unlink(file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/GTEx_heart_results.rds"))
...
markers_list <- readRDS(file.path(project_root, "results/benchmark/reference_comparison/1_preprocessing/GTEx_heart_markers.rds"))
...
for (i in 1:num_parts) {
  ...
  run_with_retry <- function(attempt = 1, max_attempts = 3) {
    tryCatch({
      results <- interactive_consensus_annotation(
        input = part_markers,
        tissuename = sprintf("human heart tissue part %d", i),
  ...
  saveRDS(results,
          file.path(project_root,
                    sprintf("results/benchmark/reference_comparison/2_evaluation/GTEx_heart_results_part%d.rds", i)))
...
# Save merged results
combined_file <- file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/GTEx_heart_results.rds")
