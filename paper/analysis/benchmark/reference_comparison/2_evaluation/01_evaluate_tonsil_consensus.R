# Evaluation script for Tonsil cell type benchmark
library(LLMCellType)
...
unlink(file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/tonsil_results.rds"))
...
markers_list <- readRDS(file.path(project_root, "results/benchmark/reference_comparison/1_preprocessing/tonsil_markers.rds"))
...
results <- interactive_consensus_annotation(
  input = markers_list,
  tissuename = "human tonsil tissue",
...
# Save results
saveRDS(results, file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/tonsil_results.rds"))

print("Evaluation complete. Results saved to tonsil_results.rds")
