# Evaluation script for GTEx Esophagus cell type benchmark
library(LLMCellType)
...
unlink(file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/GTEx_esophagus_results.rds"))
...
markers_list <- readRDS(file.path(project_root, "results/benchmark/reference_comparison/1_preprocessing/GTEx_esophagus_markers.rds"))
...
# Split markers_list into 8 parts
...
for (i in 1:num_parts) {
  ...
  # Function to run consensus annotation with retry
  run_with_retry <- function(attempt = 1, max_attempts = 3) {
    tryCatch({
      results <- interactive_consensus_annotation(
        input = part_markers,
        tissuename = sprintf("human esophagus tissue part %d", i),
      ...
      if (attempt < max_attempts) {
        message(sprintf("\nError in batch %d, attempt %d: %s\nRetrying after 10 seconds...", i, attempt, e$message))
        Sys.sleep(10)  # Wait 10 seconds before retry
        return(run_with_retry(attempt + 1, max_attempts))
      } else {
        stop(sprintf("Failed after %d attempts. Last error: %s", max_attempts, e$message))
      }
    })
  }

  # Run consensus annotation for this part with retry
  results <- run_with_retry()

  # Save results for this part
  saveRDS(results,
          file.path(project_root,
                    sprintf("results/benchmark/reference_comparison/2_evaluation/GTEx_esophagus_results_part%d.rds", i)))

  print(sprintf("Part %d evaluation complete. Results saved to GTEx_breast_results_part%d.rds", i, i))
}
...
# Save merged results
combined_file <- file.path(project_root, "results/benchmark/reference_comparison/2_evaluation/GTEx_esophagus_results.rds")
